...
uniform sampler2D sampler2d_car;
...
{                                                        
	...
	if(gl_FragCoord.x>=110.0&&gl_FragCoord.x<=220&&gl_FragCoord.y>=190&&gl_FragCoord.y<=300)
	{
		gl_FragColor = texture2D(sampler2d_car,vec2((gl_FragCoord.x-110.0)/110.0,(gl_FragCoord.y-190.0)/110.0));
	}
	else
	{
