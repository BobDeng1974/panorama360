...
bool StitchImage(StitchInfo *front, StitchInfo *rear, StitchInfo *left, StitchInfo *right)
{
	...
	//确定汽车位置
	GetCarPosition(front, rear, left, right);
...
//	cvSaveImage("simpleStich.jpg", front->stitchImage);
	//融合
...
bool WriteStitchMap(StitchInfo *front, StitchInfo *rear, StitchInfo *left, StitchInfo *right)
{
	...
	for (int x = 0; x < STITCH_IMAGE_WIDTH; ++x)
	{
		...
		{
			if (JudgeIsInside(cvPoint(x, y), front->stitchPoint, 6))//判断是否在前向图像区域
			{
				//前向
			...
			}else if (JudgeIsInside(cvPoint(x, y), rear->stitchPoint, 6))//判断是否在后向图像区域
			{
				//前向
				...
				CV_MAT_ELEM(*rightcoordinateyMat, float, y, x)	= 0;
				CV_MAT_ELEM(*rightweightMat, float, y, x)		= 0;
			}else if (JudgeIsInside(cvPoint(x, y), left->stitchPoint, 6))//判断是否在左侧图像区域
			{
				//前向
				CV_MAT_ELEM(*frontcoordinatexMat, float, y, x)	= 0;
				CV_MAT_ELEM(*frontcoordinateyMat, float, y, x)	= 0;
				...
				CV_MAT_ELEM(*rightcoordinateyMat, float, y, x)	= 0;
				CV_MAT_ELEM(*rightweightMat, float, y, x)		= 0;
			}else if (JudgeIsInside(cvPoint(x, y), right->stitchPoint, 6))//判断是否在右侧图像区域
			{
				//前向
				CV_MAT_ELEM(*frontcoordinatexMat, float, y, x)	= 0;
				CV_MAT_ELEM(*frontcoordinateyMat, float, y, x)	= 0;
...
#define MaxBlend 5			//融合区域中与拼接缝的最长矩离
	printf("start writting map\n");
	//逐点计算查找表坐标及权值
	for (int x = 0; x < STITCH_IMAGE_WIDTH; ++x)
	{
		for (int y = 0; y < STITCH_IMAGE_HEIGHT; ++y)
		{
			if (JudgeIsInside(cvPoint(x, y), front->stitchPoint, 6))//判断是否在前向图像区域
			{
				...
				D1 = abs(A1*x+B1*y+C1)/AABB1;						//计算与拼接缝1的矩离
				...
				D2 = abs(A2*x+B2*y+C2)/AABB2;						//计算与拼接缝2的矩离
				if (D1 < MaxBlend)									//在拼接缝1的融合区域
				{
					//前向
					tmppoint = cvPoint2D32f(x, y);								  //全景图中的点坐标
					TransformPoint(&tmppoint, 1, invertFrontStitchMat, &tmppoint);//计算全景图中的点在俯视图中的对应点坐标
					TransformPoint(&tmppoint, 1, invertFrontBirdMat, &tmppoint);  //计算俯视图中的点在透视图中的对应点坐标
					GetDistorPoint(&tmppoint, 1, &tmppoint, frontfx, frontfy, frontcx, frontcy, frontk1, frontk2, frontk3, frontp1, frontp2);//计算透视图中的点在原始图像中的对应点坐标
					CV_MAT_ELEM(*frontcoordinatexMat, float, y, x)	= tmppoint.x;
					CV_MAT_ELEM(*frontcoordinateyMat, float, y, x)	= tmppoint.y;
					...
					//左向
					tmppoint = cvPoint2D32f(x, y);								 //全景图中的点坐标
					TransformPoint(&tmppoint, 1, invertLeftStitchMat, &tmppoint);//计算全景点在俯视图中的对应点坐标
					TransformPoint(&tmppoint, 1, invertLeftBirdMat, &tmppoint);//计算俯视图中的点在透视图中的对应点坐标
					GetDistorPoint(&tmppoint, 1, &tmppoint, leftfx, leftfy, leftcx, leftcy, leftk1, leftk2, leftk3, leftp1, leftp2);//计算透视图中的点在原始图像中的对应点坐标
					CV_MAT_ELEM(*leftcoordinatexMat, float, y, x) = tmppoint.x;
					CV_MAT_ELEM(*leftcoordinateyMat, float, y, x) = tmppoint.y;
					...
					CV_MAT_ELEM(*rightcoordinateyMat, float, y, x)	= 0;
					CV_MAT_ELEM(*rightweightMat, float, y, x)		= 0;
				}else if (D2 < MaxBlend)							//在拼接缝2的融合区域
				{
					//前向
					tmppoint = cvPoint2D32f(x, y);								  //全景图中的点坐标
					TransformPoint(&tmppoint, 1, invertFrontStitchMat, &tmppoint);//计算全景点在俯视图中的对应点坐标
					TransformPoint(&tmppoint, 1, invertFrontBirdMat, &tmppoint);  //计算俯视图中的点在透视图中的对应点坐标
					GetDistorPoint(&tmppoint, 1, &tmppoint, frontfx, frontfy, frontcx, frontcy, frontk1, frontk2, frontk3, frontp1, frontp2);//计算透视图中的点在原始图像中的对应点坐标
					CV_MAT_ELEM(*frontcoordinatexMat, float, y, x)	= tmppoint.x;
					CV_MAT_ELEM(*frontcoordinateyMat, float, y, x)	= tmppoint.y;
					...
					//右向
					tmppoint = cvPoint2D32f(x, y);									//全景图中的点坐标
					TransformPoint(&tmppoint, 1, invertRightStitchMat, &tmppoint);	//计算全景图中的点在俯视图中的对应点坐标
					TransformPoint(&tmppoint, 1, invertRightBirdMat, &tmppoint);	//计算俯视图中的点在透视图中的对应点坐标
					GetDistorPoint(&tmppoint, 1, &tmppoint, rightfx, rightfy, rightcx, rightcy, rightk1, rightk2, rightk3, rightp1, rightp2);//计算透视图中的点在原始图像中的对应点坐标
					CV_MAT_ELEM(*rightcoordinatexMat, float, y, x)	= tmppoint.x;
					CV_MAT_ELEM(*rightcoordinateyMat, float, y, x)	= tmppoint.y;
			...
			}else if (JudgeIsInside(cvPoint(x, y), rear->stitchPoint, 6))//判断是否在后向图像区域
			{
				...
				if (D1 < MaxBlend)										//在拼接缝1的融合区域
				{
					...
					//后向
					tmppoint = cvPoint2D32f(x, y);									//全景图中的点坐标
					TransformPoint(&tmppoint, 1, invertRearStitchMat, &tmppoint);	//计算全景图中的点在俯视图中的对应点坐标
					TransformPoint(&tmppoint, 1, invertRearBirdMat, &tmppoint);		//计算俯视图中的点在透视图中的对应点坐标
					GetDistorPoint(&tmppoint, 1, &tmppoint, rearfx, rearfy, rearcx, rearcy, reark1, reark2, reark3, rearp1, rearp2);//计算透视图中的点在原始图像中的对应点坐标
					CV_MAT_ELEM(*rearcoordinatexMat, float, y, x)	= tmppoint.x;
					CV_MAT_ELEM(*rearcoordinateyMat, float, y, x)	= tmppoint.y;
					CV_MAT_ELEM(*rearweightMat, float, y, x)		= D1/MaxBlend/2+0.5;
					//左向	
					tmppoint = cvPoint2D32f(x, y);									//全景图中的点坐标
					TransformPoint(&tmppoint, 1, invertLeftStitchMat, &tmppoint);	//计算全景图中的点在俯视图中的对应点坐标
					TransformPoint(&tmppoint, 1, invertLeftBirdMat, &tmppoint);		//计算俯视图中的点在透视图中的对应点坐标
					GetDistorPoint(&tmppoint, 1, &tmppoint, leftfx, leftfy, leftcx, leftcy, leftk1, leftk2, leftk3, leftp1, leftp2);//计算透视图中的点在原始图像中的对应点坐标
					CV_MAT_ELEM(*leftcoordinatexMat, float, y, x) = tmppoint.x;
					CV_MAT_ELEM(*leftcoordinateyMat, float, y, x) = tmppoint.y;
...
bool GetCarPosition(StitchInfo *front, StitchInfo *rear, StitchInfo *left, StitchInfo *right)
{
	...
	CvPoint2D32f tmpmin, tmpmax;
	tmpmin.x = MIN(front->stitchPoint[4].x, MIN(front->stitchPoint[5].x, MIN(rear->stitchPoint[4].x, MIN(rear->stitchPoint[5].x, 
			MIN(left->stitchPoint[4].x, MIN(left->stitchPoint[5].x, MIN(right->stitchPoint[4].x, right->stitchPoint[5].x)))))));
	tmpmin.y = MIN(front->stitchPoint[4].y, MIN(front->stitchPoint[5].y, MIN(rear->stitchPoint[4].y, MIN(rear->stitchPoint[5].y, 
		MIN(left->stitchPoint[4].y, MIN(left->stitchPoint[5].y, MIN(right->stitchPoint[4].y, right->stitchPoint[5].y)))))));
	tmpmax.x = MAX(front->stitchPoint[4].x, MAX(front->stitchPoint[5].x, MAX(rear->stitchPoint[4].x, MAX(rear->stitchPoint[5].x, 
		MAX(left->stitchPoint[4].x, MAX(left->stitchPoint[5].x, MAX(right->stitchPoint[4].x, right->stitchPoint[5].x)))))));
	tmpmax.y = MAX(front->stitchPoint[4].y, MAX(front->stitchPoint[5].y, MAX(rear->stitchPoint[4].y, MAX(rear->stitchPoint[5].y, 
		MAX(left->stitchPoint[4].y, MAX(left->stitchPoint[5].y, MAX(right->stitchPoint[4].y, right->stitchPoint[5].y)))))));
	front->stitchPoint[4].x = tmpmax.x;
	front->stitchPoint[4].y = tmpmin.y;
	front->stitchPoint[5].x = tmpmin.x;
	front->stitchPoint[5].y = tmpmin.y;
	rear->stitchPoint[4].x = tmpmin.x;
	rear->stitchPoint[4].y = tmpmax.y;
	rear->stitchPoint[5].x = tmpmax.x;
	rear->stitchPoint[5].y = tmpmax.y;
	left->stitchPoint[4].x = tmpmin.x;
	left->stitchPoint[4].y = tmpmin.y;
	left->stitchPoint[5].x = tmpmin.x;
	left->stitchPoint[5].y = tmpmax.y;
	right->stitchPoint[4].x = tmpmax.x;
	right->stitchPoint[4].y = tmpmax.y;
	right->stitchPoint[5].x = tmpmax.x;
	right->stitchPoint[5].y = tmpmin.y;
	return true;
}
