		...
		CvMat* matrix, float scale)
{
	...
	if (!GetCorners(srcImage, corners, &corner_count, board_w, board_h))
	{
	...
	sortcorner(corners, corner_count);
	GetBirdMatrix(srcImage, birdImage, corners, &matrix, scale, board_w, board_h);
	cvWarpPerspective(srcImage, birdImage, matrix, CV_INTER_LINEAR | CV_WARP_FILL_OUTLIERS , cvScalarAll(0));
...
bool GetCorners(IplImage* srcImage, CvPoint2D32f* corners, int *corner_count, int board_w, int board_h)
{
	...
	int found = cvFindChessboardCorners(srcImage,cvSize(board_w, board_h), 
		corners, corner_count, CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
	if (!found)
	{
		return false;
	}
...
/*              求DLT变换矩阵                                           */
/************************************************************************/
bool lsq_homog( CvPoint2D64f* pts, CvPoint2D64f* mpts, int n, CvMat *H)
{
	CvMat *A, * B, X;
		...
		CvMat **matrix, float scale, int board_w, int board_h)
{
	...
	CvPoint2D64f objPts[4], imgPts[4];
...
AdjustScale:	objPts[0] = cvPoint2D64f(0, 0);
	...
	lsq_homog(imgPts, objPts, 4, *matrix);
	...
	TransformPoint(src, 4, *matrix, dstpoint);
	...
	CvMat **leftStitchMat, CvMat **rightStitchMat, IplImage *stitchImage)
{
	...
	lsq_homog(srcpoint, dstpoint, 4, *frontStitchMat);
	//计算后向图像映射矩阵
	...
	lsq_homog(srcpoint, dstpoint, 4, *rearStitchMat);
	//计算左向图像映射矩阵
	...
	lsq_homog(srcpoint, dstpoint, 4, *leftStitchMat);
	//计算右向图像映射矩阵
	...
	lsq_homog(srcpoint, dstpoint, 4, *rightStitchMat);
...
bool GetRotationMat(float angle, CvMat **rotationMat, int imagewidth, int imageheight)
{
	...
	lsq_homog(srcpoint, dstpoint, 4, *rotationMat);
...
bool GetMoveMat(int xdistance, int ydistance, CvMat **moveMat)
{
	...
	lsq_homog(srcpoint, dstpoint, 4, *moveMat);
